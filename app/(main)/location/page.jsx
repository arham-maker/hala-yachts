...
async function getLocationsFromMongoDB() {
  const client = new MongoClient(process.env.MONGODB_URI || 'mongodb://localhost:27017');
  
  try {
    await client.connect();
    const database = client.db('halayachts-database');  
...
export default async function Location() {
  // Fetch data from MongoDB instead of JSON file
  const locationsData = await getLocationsFromMongoDB();
